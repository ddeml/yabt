...
Restore uses the same durable metadata that backup creates. The root change manifest records the root format so restore can distinguish a packaged root from a packaged child without trusting mutable local policy. A current-live restore pre-stages and hash-validates every file that it needs to write before the first destination mutation. It leaves identical files alone and moves changed or extra destination representations to destination history before installing the staged content. Source `stat-v1` timestamps are restored exactly for mirror files. ZIP entry timestamps use the standard DOS timestamp field: restore interprets the stored wall-clock value as UTC, but its two-second resolution rounds down odd seconds and discards fractional seconds. Any destination live change manifest is invalidated and removed when restore changes live state, because it would otherwise describe the pre-restore bytes. Restoring a specific logical history version remains future work.
