...
When a mutating backup must compare streams, it copies the source bytes it is already comparing into a private delete-on-close local replay file. If the comparison finds a change, YABT finishes that one source read and uploads the captured snapshot instead of reopening the source. This both avoids duplicate source I/O and prevents a file changed between comparison and upload from producing a different archived version. Objects are currently processed sequentially, so local temporary storage must hold at most the largest projected object being compared. Verify and dry-run operations do not create replay files because they never upload. ZIP follows the same one-read rule: when an entry lacks enough metadata or a provider hash, its bytes are hashed while that same read is written into the package. ZIP package construction attempts every independent source entry and aggregates source-read failures. The initial ZIP builder remains memory-backed; an eagerly built fallback package retains one backing buffer for replay, so nested incomplete-metadata packages can temporarily retain multiple buffers until their projection graph is released. A future large-package implementation needs an explicitly owned, spillable content lifetime.

Backup classifies source-object open and stream-read failures separately from target-provider or mutation failures. It continues reconciliation after independent source read failures and returns a deterministic end-of-run report with every source path and underlying reason. A failed replacement is never historized before its readable source snapshot exists. If any source read fails, target extras, root-descriptor replacement, history finalization, and live change-manifest publication are deferred; any successful readable-object writes remain conservative rerun checkpoints and invalidation evidence remains active where required. Cancellation, enumeration, target reads/writes, locks, and historization failures remain fail-fast.
...
Restore uses the same durable metadata that backup creates. The live change manifest records the root format and projection groups, package manifests describe each reversible ZIP representation, and the archived root descriptor supplies the authoritative archive identity and layout. Final files flow through a completion-order pipeline bounded by `Sync:RestoreMaximumConcurrency`, default `5`. Each new or changed file is written once to a unique staging file under the destination's reserved `.yabt-tmp` while its length and xxHash128 value are validated. Only then does restore historize that file's blocking live representation and atomically rename the staging file into place. Format-projection staging is released after its last dependent output is prepared. Unrelated extras are historized after all desired files succeed.

This per-file transaction is the restore checkpoint. Archive-file and destination-file read failures are collected across the bounded pipeline and returned as one deterministic path-and-reason report. Earlier committed files remain live, each failing file's previous representation remains untouched, unrelated extras remain live, and invalidation markers keep incomplete metadata untrusted. No logical-state manifest is published until every required file succeeds. The next run hashes uncertain destination files and skips those that already match instead of downloading them again. Temporary final-output storage is bounded by the configured in-flight files rather than the sum of all changed data. Commit, staging, historization, mutation-lock, and metadata-write failures remain fail-fast because continuation would be unsafe.

After a successful restore, the filesystem root contains a separate plain `.yabt-logical-state-manifest.json`. Its self-hashed entries bind each logical file path's current canonical `stat-v1` fingerprint to the desired actual-byte content hash. On a later normal restore, a matching stat fingerprint and desired hash allow YABT to leave that file untouched without opening it. Missing, invalid, conflicting, or invalidated evidence falls back to hashing the destination file. `restore --byte-for-byte` bypasses this shortcut; unchanged outputs are streamed through validation while changed outputs use the same single staging read that will be committed. Restore uses `.yabt-logical-state-manifest.invalid` as a transaction marker, publishes rebuilt evidence after the live filesystem is correct, and removes the marker last.
