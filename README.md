...
If an Azure archive was created by an earlier development build, run one current `backup` before the first restore. This upgrades its fieldless schema-version-1 change manifest to version 2 with durable root-format evidence; unchanged archive files are not re-uploaded.
...
Restore reads the current logical live branch from the selected archive store. The `source-root` must be the folder containing `.yabt-root.json`; use `--target-store-id` when that descriptor contains multiple stores. Restore validates the live change manifest, uses its durable root format, safely extracts YABT ZIP packages, and reconciles the filesystem destination to that live state. Identical files remain untouched. Before changing the destination, YABT stages and hash-validates every file that must be written; temporary disk space up to the total size of those new and changed files may therefore be required. Replaced files, extra files, extra folders, and file/folder shape conflicts move to one new timestamped version in the destination's `.yabt-hist` branch before staged content is installed. Original mirror-file modification times are restored. ZIP restores preserve the stored timestamp as UTC, but the standard ZIP DOS timestamp has only two-second precision, so odd or fractional seconds cannot be recovered. The descriptor and provider plumbing remain internal and are preserved, while any stale destination live change manifest is safely invalidated and removed. Restore never reads historical archive versions yet; selecting a historical version remains future work.
