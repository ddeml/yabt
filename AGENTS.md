...
Normal restore may trust a valid destination logical-state entry without opening the destination file only when the current filesystem `stat-v1` fingerprint matches and the desired content hash equals the recorded hash. Otherwise hash the destination file. `restore --byte-for-byte` bypasses this shortcut and validates every desired archive output in the same bounded per-file work pipeline. Stage each new or changed file once under the destination's reserved `.yabt-tmp`, calculate and verify its length and content hash during that write, historize only the live representation that blocks that validated file, and atomically move the staging file into place. Bound concurrent preparation and temporary storage by `RestoreMaximumConcurrency`, which defaults to 5. Defer unrelated extra-item historization until every desired file has been prepared and committed. A successfully committed file is a rerun checkpoint: interrupted restore leaves invalidation evidence active, and the next run hashes destination state and skips files that already match. Release format-projection staging when its last dependent output has been prepared. When restore changes live state and prior logical-state evidence exists, create the invalidation marker immediately before the first live mutation; publish a rebuilt manifest after the filesystem is reconciled and delete the marker last. A leftover marker or invalid document disables quick evidence and causes content reads on the next restore.
